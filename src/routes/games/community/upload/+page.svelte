    ...
    async function executeUpload() {
        ...
        try {
            ...
            if (result.success) {
                toast(
                    "Game Uploaded",
                    "Your community game is now live!",
                    "check_circle",
                    4000,
                    "success"
                );
...
{#if showConfirmModal}
    <Modal 
        ...
        <p>
            ...
            Currently, you cannot edit the game files after uploading. Ensure your <strong>index.html</strong> is at the root of the ZIP file.
        </p>
...
<Flex justifyContent="center" alignItems="center" height="calc(100dvh - 56px)">
    <Form width="500px" onsubmit={requestUpload}>
        ...
        {#if files && files.length > 0}
            <Flex direction="column" gap="xsmall" marginTop="small" marginBottom="medium">
                ...
                {#each Array.from(files) as file}
                    <Flex alignItems="center" gap="small" style="padding: 8px; background: {token.theme.color.background.secondary}; border-radius: 6px;">
                        <Icon icon="folder_zip" />
                        <span style="flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{file.name}</span>
                        <span style="color: {token.theme.color.text.tertiary}; font-size: 0.8rem;">{formatBytes(file.size)}</span>
                    </Flex>
                {/each}
            </Flex>
        {/if}
        ...
        <Flex justifyContent="end" marginTop="medium">
            <Button form= type="submit" loading={isUploading} disabled={isUploading}>Review & Upload</Button>
        </Flex>
    </Form>
</Flex>